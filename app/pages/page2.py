...
def page2_reader(q, details=None):
    data = {'idea': q.args._idea,
            'summary_st': q.args._opt1} 
    return data 
...
def page2_search_box(q, details=None):
    items = [
        # ui.text_l("Search Query"),
        ui.textbox(name='_idea', label='Idea', icon='Play', spellcheck=True, placeholder='General Domain used in Metaphor', multiline=True, value="falling love with distance girl, but don't know how to talk to her at least."),
        # ui.choice_group(name='_opt1', label='Get Summary', choices=[
        #     ui.choice(name=x, label=x) for x in ['Yes', 'No']
        # ], required=True, inline=True, value='Subject-Domain'),
        ui.toggle(name='_opt1', label='Get Summary', value=False),
        ui.buttons([ui.button(name='page2_search', label='Search', primary=True, icon='DocumentSearch')], justify='center')
    ]
    card = ui.form_card(
        box=ui.box('content_11', order=0, height='220px'),
        items=items
    )
    return card

def page2_stats(q, details=None):
    try:
        time_s = q.client.record_data2['time']
        ...
        records = q.client.record_data2['#records']
        ...
        singers = q.client.record_data2['#records']
        ...
        writers = q.client.record_data2['#writers']
        ...
        composers = q.client.record_data2['#composers']
...
def page2_side_bar(q, details=None):
    try:
        ...
        stat_cards = [
            ui.stat_list_item(label=i, name=f'SP2-{i}', icon='CodeEdit', icon_color='blue')
            for i in songs
        ]
...
def page2_content(q, details=None):
    song_name = q.client.page2_song 
    if song_name != None:
...
def page2_topic_corr(q, details=None):
    try:
        topics, freq = q.client.record_data2['topic_similarities']
        items = []
...
def page2_advance_data(q, details=None):
    try:
        all_data = q.client.record_data2['all_data']
...
async def page2_loader(q, details=None):

    # get search option
    search_card = page2_search_box(q, details)

    # get side bar card 
    side_card = page2_side_bar(q, details)

    # get query search status cards 
    st_cards = page2_stats(q, details)

    # get advance stats
    adv_card = page2_advance_data(q, details)

    # get topic correlations 
    topic_corr = page2_topic_corr(q, details)

    # get song content 
    content_card = page2_content(q, details)
...
# ==========================================================================
# ===================== Page-2 Functionalities =============================

def page2_qeury_processor(q, details=None):
    data = q.client.details
    idea = data['idea']
    if data['summary_st'] == 'Yes':
        idea = textSummarization(idea)
    ...
    topic_similarities = textSimilarity([idea]+topics)
    matching_topic = topics[np.argmax(topic_similarities, axis=0)]


    q = {"match": {'topic': matching_topic}}
    t1 = datetime.now()
    resp = query_search(q)
    t2 = datetime.now()
    # search information
    ...
    result = {
        ...
        # search result data
        "idea": idea,
        "topic_similarities": (topics, topic_similarities.tolist()),
...
def get_song(q, name):
    data = q.client.record_data2['primary_data']
    for r in data:
        if r['title'] == name:
            return r 
    else:
